...
- 20. 先决条件：先完成 1～3 步中的准备工作，且机器自检（SELF-TEST）过程无异常；
- 21. 进入设备 SETTINGS 页，进入 CURRENT LIMIT SETTING 子菜单，并使用「上下左右」导航按钮到 I LIMIT：字段；
  
  > 预期结果：I LIMIT：字段的 MAX = 和 MIN = 子字段开始闪烁。

- 22. 在 I LIMIT：MAX 字段中填写预期的电流最大值；在 I LIMIT：MIN 字段中填写预期电流的最小值。
...
- 24. 仿照 步骤 8～步骤 11 的方式，将机器的输出电压更改为超出电流墙限制范围的值；
...
> 
> 为电流和电压设定摆率上限是很有必要的，因为过于剧烈的电压或电流变化很可能造成永久性的数字电路损坏。
